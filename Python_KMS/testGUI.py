...
def insertData():
    ...
    try :
        ...
        messagebox.showerror('Error', 'DataInput Error')
    else :
        messagebox.showinfo('Input Success', 'DataInput Suceesful')
...
def selectData() :
    ...
    # print : Feild Name
    strData1.append("ID");  strData2.append("Name")
...
def searchData() :
    ...
    data1 = "aa"; data2 = edt2.get(); data3 = edt3.get(); data4 = edt4.get()

    while(True) :
        row = cur.fetchone()
        if row == None :
            break
        if data1 == row[0] or data2 == row[1] or data3 == row[2] or data4 == row[3] :
            strData1.append(row[0]); strData2.append(row[1])
            strData3.append(row[2]); strData4.append(row[3])
...
btnSearch = Button(edtFrame, text="Find", command=searchData)
